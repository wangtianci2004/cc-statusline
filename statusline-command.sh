...
# Git 分支与工作树状态（dirty ●，staged +n，ahead ↑n，behind ↓n）
...
if [ -n "$git_branch" ]; then
  ...
  if [ -n "$_porc" ]; then
    _staged=$(printf '%s\n' "$_porc" | awk 'BEGIN{n=0} /^[MADRCU]/{n++} END{print n}')
    _dirty=$(printf '%s\n' "$_porc" | awk 'BEGIN{n=0} /^.[MD?]/{n++} END{print n}')
    [ "$_dirty" -gt 0 ] && _gst="${_gst}'"
    [ "$_staged" -gt 0 ] && _gst="${_gst}+${_staged}"
  fi
  _ab=$(git -C "$cwd" rev-list --left-right --count '@{u}...HEAD' 2>/dev/null)
  if [ -n "$_ab" ]; then
    _behind=$(echo "$_ab" | awk '{print $1}')
    _ahead=$(echo "$_ab" | awk '{print $2}')
    [ "$_ahead" -gt 0 ] 2>/dev/null && _gst="${_gst}↑${_ahead}"
    [ "$_behind" -gt 0 ] 2>/dev/null && _gst="${_gst}↓${_behind}"
  fi
  branch_tag=" ${GRN}${git_branch}${_gst}${RST}"
fi
...
# ANSI 颜色：值默认绿色；阈值告警标红加粗
RED=$(printf '\033[1;31m')
GRN=$(printf '\033[32m')
RST=$(printf '\033[0m')

# 百分比格式化：四舍五入到 1 位小数，去掉末尾 .0（55 而非 55.0）
fmt_pct() {
  awk -v n="$1" 'BEGIN{
    s=sprintf("%.1f", n+0)
    sub(/\.0$/, "", s)
